...
void ABaseCharacter::Tick(float DeltaTime)
{
    ...
    if (TheEndGame) {
        if (!TheEndGameOn) {
           // //GEngine->AddOnScreenDebugMessage(-1, 8, FColor::Purple, FString::Printf(TEXT("endGame: %d"), IsLocallyControlled()), true, FVector2D(1, 1));
            //UE_LOG(LogTemp, Warning, TEXT("777777777777777777777777777"));
            //      Ȱ  ȭ                   ɴϴ .
...
//void ABaseCharacter::OnDieProcess()
//{
//   if (ANetPlayerController* NetPC = GetController<ANetPlayerController>())
//   {
//      NetPC->ServerChangePlayerToSpectator();
    ...
    if (CurrHP <= 0) {
        if (HasAuthority()) {
            if (otherPlayer) {
                ...
                /*int32 myScore = GetPlayerState()->GetScore();
                int32 EnemySc:ServerDamagedHealthore = otherPlayer->GetPlayerState()->GetScore();
                Score=(myScore < EnemyScore) ? otherPlayer->GetPlayerState()->GetScore() : GetPlayerState()->GetScore();*/

                //UE_LOG(LogTemp, Warning, TEXT("Score is %"), Score);
                ...
                if (otherPlayer->GetPlayerState()->GetScore() >= 3)
                {
                    ...
                    /*if (HasAuthority()) {
                       Server_AllEndGame_Implementation();
...
void ABaseCharacter::Stun()
{
    ...
    //UE_LOG(LogTemp, Warning, TEXT("%d"), CurrHP);
    ...
    for (int i = 0; i < playerArray.Num(); i++)
    {
       ...
       // p->TheEndGame = true;
        ...
        if (p->GetController() != nullptr && p->IsLocallyControlled() == true) {
            //GEngine->AddOnScreenDebugMessage(-1, 7, FColor::Purple, FString::Printf(TEXT("p->CurrHP: %f / MyScore: %f"), p->CurrHP, p->GetPlayerState()->GetScore()), true, FVector2D(1, 1));
            if (p->CurrHP <= 20) {
                //
                if (!TheEndGame) {
               ...
               // GetWorldTimerManager().SetTimer(WinTimerHandle, FTimerDelegate::CreateLambda([&]() {
               //   //  if (IsLocallyControlled()) {
               //       winloseInstance = CreateWidget<UWinLoseWidget>(GetWorld(), WinLoseWidgetFactory);
               //             if (winloseInstance) {
               //                 winloseInstance->AddToViewport(1);
               //             }
               //             //GEngine->AddOnScreenDebugMessage(-1, 3, FColor::Purple, FString::Printf(TEXT("Create Win Widget")), true, FVector2D(1, 1));
               //     //    }
               //     }), 8.0f, false);
...
void ABaseCharacter::NotifyRestarted()
{
    ...
    if (WinLoseWidgetFactory) {
        ...
        if (IsLocallyControlled()) {
            ////GEngine->AddOnScreenDebugMessage(-1, 3, FColor::Purple, FString::Printf(TEXT("NotifyRestrat")), true, FVector2D(1, 1));
            winloseInstance = CreateWidget<UWinLoseWidget>(GetWorld(), WinLoseWidgetFactory);
            if (winloseInstance) {
                winloseInstance->AddToViewport(1);
                ////GEngine->AddOnScreenDebugMessage(-1, 8, FColor::Purple, FString::Printf(TEXT("11111111111111111111")), true, FVector2D(1, 1));
            }
...
//void ABaseCharacter::Multicast_DieShowMyMesh_Implementation()
//{
//   if (IsLocallyControlled()) {
//      GetMesh()->SetVisibility(true);
//      fpsMesh->SetVisibility(false);
